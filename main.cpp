...
Vector3 ProjectWorldToScreen69(Vector3 WorldLocation, Vector3 camrot)
{
	...
	uint64_t chain = read<uint64_t>(Localplayer + 0x80);
	uint64_t chain1 = read<uint64_t>(chain + 0x98);
	uint64_t chain2 = read<uint64_t>(chain1 + 0x160);
...
void renderLoopCall() {
	...
	if (Options.ShowMenu)
	{
		...
		if (ImGui::Begin("UwU", NULL, 58))
		{
			...
			ImGui::Checkbox(XorStr("Silent Aimbot").c_str(), &Options.Visuals.guntest2);
			...
			ImGui::Checkbox(XorStr("Skyfly1").c_str(), &Options.Visuals.guntest3);///////// OUTDATED
...
void mainy()
{
	...
	Sleep(2000);
	Beep(888, 500);
	SetConsoleTitleA(XorStr("UwU Undetected Cheat Loaded").c_str());
	system(XorStr("cls").c_str());
